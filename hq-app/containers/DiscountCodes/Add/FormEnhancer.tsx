...
const DiscountFormEnhancer = ({
  ...
  productOtions = productOtions?.map((items: any) => {
    return {
      id: items?.productId,
  ...
  itemGroupOtions = itemGroupOtions?.map((items: any) => {
    return {
      id: items?.itemGroupId,
      name: items?.itemGroupName,
  ...
  storeOtions = storeOtions?.map((items: any) => {
    return {
      id: items?.storeId,
  ...
  const formProps = useForm<ADD_UPDATE_FROM_PROPS>({
    ...
    defaultValues: id
      ? {
          discountItemGroups: itemGroupOtions,
          discountStores: storeOtions,
          discountProducts: productOtions,
        }
      : formInitialValues,
  });
  ...
  useEffect(() => {
    ...
    setValue('discountItemGroups', itemGroupOtions);
    setValue('discountStores', storeOtions);
    setValue('discountProducts', productOtions);
  ...
  const _handleSubmit = async () => {
    ...
    if (id) {
      ...
      const response: {
        error?: FetchBaseQueryError | SerializedError;
        data?: { id: { [key: string]: string | number } };
      } = await createDiscount(payloadUpdate);

      if (response?.data) {
        window.location.href = `/settings/discountcode/${response?.data?.id}`;
